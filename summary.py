...
def parse_rows(rows: list[list[str]]) -> tuple[dict[int, int], int, dict[int, bool]]:
    ...
    return solved_by_group, max_pid, solved_by_pid


def build_summary(
    solved_by_group: dict[int, int], max_pid: int, solved_by_pid: dict[int, bool]
) -> list[str]:
    if max_pid == 0:
    ...
    for start in range(1, max_group_start + 1, 100):
        ...
        groups.append((start, end, solved, total, missing))
...
def compute_summary_lines(lines: list[str]) -> list[str]:
    ...
    solved_by_group, max_pid, solved_by_pid = parse_rows(rows)
    return build_summary(solved_by_group, max_pid, solved_by_pid)
