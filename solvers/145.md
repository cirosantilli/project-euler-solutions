...
The implementation does not brute-force all integers below one billion. It uses the carry analysis above to count complete digit lengths directly.

### Closed-form count by length

For an even length $2k$, every carry is forced to be zero, so the count is

$$R_{2k}=20\cdot 30^{k-1}.$$

For an odd length, only lengths $4m+3$ contribute, and their count is

$$R_{4m+3}=100\cdot 500^m.$$

All lengths $4m+1$ contribute zero. The program sums these formulas for each complete decimal length below the requested limit.

### Small fallback for partial limits

The Euler target is exactly $10^9$, and the checkpoint is exactly $1000$, so both are handled entirely by complete-length formulas. The helper keeps a small brute-force range counter for non-power-of-ten limits, but it is not used by the main run.

## Complexity Analysis

For power-of-ten limits, the runtime is $O(D)$ where $D$ is the number of decimal lengths considered. Here $D=9$, so the computation is effectively constant time and uses $O(1)$ memory. A non-power-of-ten limit may additionally use the brute-force fallback on the final partial length.
