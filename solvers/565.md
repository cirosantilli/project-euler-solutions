...
Next, the implementation enumerates all trigger pairs $(q,e)$. For the order-2 branch it records exponent $1$ from the progression sieve. For the other branch it computes the multiplicative order as a divisor of $2016$, lists every exponent $e=m\operatorname{ord}_{2017}(q)-1$ with $q^e\le N$, and discards primes that cannot contribute.

Once the trigger list is known, the implementation accumulates all single-event formulas above. It then subtracts all admissible pair overlaps in three groups: order-2 with order-2, order-2 with other triggers, and other triggers with each other. All of this is done directly in Python with integer arithmetic.
