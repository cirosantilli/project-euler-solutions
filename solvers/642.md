...
The Python implementation keeps the compressed prime-sum table in indexed
lists rather than dictionaries.  During recursion it uses the prime index as
the lower bound, memoizes only states where another prime square can still fit,
and evaluates terminal states directly from the prime-sum table.  The recursion
performs many square-root boundary checks; for this fixed input all bounds are
below \(2^{53}\), so a float-seeded floor square root is exact and avoids the
slower general-purpose integer routine in that hot loop.
