...
### 2) Memoized expectation recurrence
...
The current Python implementation uses the direct recurrence:

```text
E(state) = 1 + sum P(non-stopping transition) * E(next_state)
```

The branch that draws the current rank stops immediately, so it contributes to the leading `1` draw but has no recursive term. Each transition removes one card, and `lru_cache` ensures every reachable histogram state is evaluated once.

### 3) Floating-point evaluation and final rounding

The reachable state graph has only a few thousand states and the recurrence is numerically stable for this deck, so double-precision arithmetic is sufficient for the required eight decimal places. The result is formatted with exactly eight digits after the decimal point.
