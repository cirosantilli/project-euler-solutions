...
Instead, we build a **wheel modulus** from small primes:
...
The primes `2,3,5,7` still matter here because the expressions being tested are
the reduced quotients, not the original nine consecutive numbers.
...
This is a Python-friendly hybrid of the sieve idea: the CRT wheel handles the
small-prime progressions that would otherwise dominate the candidate stream, and
Miller-Rabin certifies the much smaller set of survivors.  A few additional
trial-divisibility checks on the four largest terms reject obvious composites
before the Miller-Rabin calls.
