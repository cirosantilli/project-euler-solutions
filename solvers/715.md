...
This repository contains a single-file Python solution for Project Euler 715.
...
The implementation also handles terminal states directly when the next
available prime cannot form a composite factor; then only the precomputed
prime-prefix table is needed, so no memo entry or recursive call is created.
...
These appear as assertions in `715.py`.
