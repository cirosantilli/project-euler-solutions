...
The Python implementation also keeps two small constant-factor details explicit: the branch roots
are refined with a fixed bisection count that is already beyond the precision needed for the
rational search, and Euclid pairs with the same parity are reduced by the known factor `2` instead
of running a full `gcd` on every candidate triple.
