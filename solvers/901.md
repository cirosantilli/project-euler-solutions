...
Under the recurrence, the expected-time sum collapses into

\[
C(x)=x+\sum_{j\ge 0} e^{-d_j}
\]

where \(x=d_1\). This avoids multiplying enormous depths by tiny probabilities and makes the computation numerically stable.

The series is truncated once the next term is far below what could affect 9-digit rounding.

### 4) Boundary search on the admissible branch
The local implementation follows the branch-search approach:

- simulate the recurrence and reject a trial \(x\) as soon as a non-positive gap appears,
- coarsely scan \(0.6 \le x \le 1.5\) to bracket the failure-to-success transition on the upper admissible branch,
- use bisection to isolate the smallest admissible \(x\) on that branch,
- evaluate the simplified cost sum at that boundary.

Double precision is sufficient here because the recurrence escapes quickly after the boundary and the final answer is rounded to 9 decimal places. All steps use Python’s standard library only.
