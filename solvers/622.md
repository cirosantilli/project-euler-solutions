...
The implementation follows the divisor pipeline directly. It first factors $2^{60}-1$ and then generates every divisor from the resulting prime powers. For this fixed Euler input the factors are small, so a simple trial division that divides the remaining cofactor as soon as a prime is found reaches the full factorization quickly; the important point is that the program no longer tests every integer up to $\sqrt{2^{60}-1}$ as a possible divisor.

For each generated divisor, the implementation performs fast modular exponentiation for the exact-order filter. Since the divisor already divides $2^{60}-1$, it only has to reject the three smaller exponents forced by the prime divisors of $60$:

$$30,\qquad 20,\qquad 12.$$

Any divisor that passes those tests contributes $d+1$ to the running sum. Before computing the target case $60$, the code checks the smaller order-$8$ example whose total is $412$, providing a direct sanity check that the divisor generation and order filter agree.
