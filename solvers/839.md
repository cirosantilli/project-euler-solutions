...
### 3) Count moves via prefix sums
Define the prefix sums

- `P_k = a[0] + ... + a[k]`

A single move transfers one bean from `i` to `i+1`, decreasing exactly the prefixes ending at `i` by **1** and leaving all other prefixes unchanged. Therefore:

- `B(N) = sum(prefix(initial)) - sum(prefix(final))`

This matches the transport cost directly and avoids tracking per-boundary flow or simulating steps.
...
- After processing all `N`, computes the final prefix-sum total directly from blocks using arithmetic-series formulas.
- Prints `B(10^7)`.
