...
The solver asserts this directly with the checkpoint $F(3)=0$.
...
This is exactly what the local `options` helper returns: `Filled`, `Horizontal`, or `Vertical`, together with the bit updates needed for the current occupancy mask and for the outgoing mask.
...
The Python solver stores the row frontier in a dictionary keyed by the packed profile state `in1 | (in2 << n)`. A cached `transitions` function expands one input state and the last-row flag into all possible output states for that row. It fills the row left to right, generates the legal options for each matching layer, and rejects only the two edge-sharing conflicts: both layers choosing the same horizontal edge or both choosing the same vertical edge.

After a row transition is known, the outer DP only multiplies the cached transition multiplicities by the number of ways to reach the input state. The code asserts $F(2)=2$, $F(3)=0$, and $F(4)=88$ before evaluating the target $F(10)$:
