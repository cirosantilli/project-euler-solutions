...
The Python DP keeps the seven residue counters for each state in mutable lists
while advancing a length.  They are not shared between states, so this avoids
the cost of rebuilding millions of short tuples in the hot transition loop.
