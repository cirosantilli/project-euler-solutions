...
The C++, Python, and Java implementations follow the recurrence directly. They first precompute or update the powers $10^0,10^1,\dots,10^{19}$ modulo $10^9$, because each transition needs the place value of the newly added digit. The Python implementation only keeps the current and next square-sum rows for the count and value tables, initializes the empty-string base state, and iterates through all lengths from 1 to 20.
...
The Python implementation keeps only rolling current/next rows for the two tables, so the memory usage is

$$O(81L).$$

Even a full two-dimensional table would be tiny compared with brute force over $10^{20}$ numbers, but the rolling arrays avoid carrying unused earlier lengths.
