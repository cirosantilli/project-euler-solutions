...
The implementation first precomputes factorials up to the required length. The recursive search chooses a count for the current digit from $0$ up to $\min(R,\text{remaining})$, multiplies the running denominator by the corresponding factorial, and moves to the next digit. It does not need to store the whole count vector: the running denominator and the chosen zero count are enough to evaluate a leaf.

The search also prunes branches where the remaining slots cannot fit into the digits left under the repetition cap. For Euler 172 this leaves only the bounded digit-count vectors, rather than the much larger position-by-position state space.

### Leaf Evaluation and Leading-Zero Correction

At a valid leaf, the implementation uses the simplified contribution

$$N(c)=\frac{(L-c_0)(L-1)!}{\prod_{d=0}^{9} c_d!}.$$

This folds the leading-zero correction into one integer division, so each admissible count vector contributes in $O(1)$ time. This is why the program is short: once the correct count-vector formula is known, the implementation is just a bounded DFS together with constant-time arithmetic at each admissible leaf.
