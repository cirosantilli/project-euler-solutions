...
### 3) Fast rank lookup and 32-bit Fenwick arrays
The generated values are below `10,000,019`, so the implementation builds a
direct value-to-rank table after marking the values that appear.  This avoids
sorting index objects while still using compressed ranks for the Fenwick trees.

Each Fenwick level stores two `array('I')` buffers: one for counts and one for
sums.  Both are reduced modulo `1,000,000,007`, so 32-bit storage is sufficient.
